...
export function createAdminRepository(config, client = null) {
  ...
  return {
    ...
    async getConversion(filters = {}) { return rpc("admin_dashboard_conversion", {
      p_processing_type: filters.processingType || null,
      p_phone_suffix: filters.phoneSuffix || null,
      p_period: filters.period || "24h",
    }); },
    ...
    async loginGuard(clientKey) {
      return rpc("admin_login_guard", { p_client_key: clientKey, p_max_failures: 5 });
    },
    async recordLogin(clientKey, success) {
      return rpc("admin_login_record", { p_client_key: clientKey, p_success: success, p_window_minutes: 15 });
    },
    async health() {
...
export function unauthorized() {
  return jsonResponse({ ok: false, error: "No autenticado", code: "UNAUTHORIZED" }, 401);
}

/*
 * Devuelve `{ ok: true, session, repository }` o `{ ok: false, response }`.
 * Comprobar la revocación toca Supabase, así que puede fallar: cuando eso pasa
 * respondemos 503 controlado en lugar de dejar escapar la excepción.
 */
export async function requireAdmin(request, config, repository = createAdminRepository(config)) {
  ...
  try {
    if (await repository.isSessionRevoked(session)) return { ok: false, response: unauthorized() };
  } catch (error) {
    console.error(JSON.stringify({ event: "admin_session_check_error", code: error?.code || "SUPABASE_ERROR" }));
    return { ok: false, response: adminError("SUPABASE_ERROR", 503) };
  }
